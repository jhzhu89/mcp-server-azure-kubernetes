...
export async function kubectlLogs(
  ...
  try {
    ...
    if (resourceType === "pod") {
      ...
      try {
        const result = execSync(baseCommand, {
          encoding: "utf8",
      ...
      if (resourceType === "deployment") {
        ...
        try {
          const jobs = execSync(jobsCommand, {
            encoding: "utf8",
            env: { ...process.env, KUBECONFIG: kubeconfigPath },
          })
            .trim()
      ...
      try {
        if (resourceType === "deployment") {
          ...
          const selectorJson = execSync(selectorCommand, {
            encoding: "utf8",
            env: { ...process.env, KUBECONFIG: kubeconfigPath },
          }).trim();
...
async function getLabelSelectorLogs(
  ...
  try {
    ...
    const pods = execSync(podsCommand, {
      encoding: "utf8",
      env: { ...process.env, KUBECONFIG: kubeconfigPath },
    })
      .trim()
    ...
    for (const pod of pods) {
      ...
      try {
        const logs = execSync(podCommand, {
          encoding: "utf8",
          env: { ...process.env, KUBECONFIG: kubeconfigPath },
        });
        logsMap[pod] = logs;
